...
.dropdown-menu {
    min-width: auto; /* Menghapus lebar minimum default */
                ...
                <div class="tab-content" id="orderTabContent">
                    ...
                    <div class="tab-pane fade" id="payment-notes" role="tabpanel" aria-labelledby="payment-notes-tab">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="total_berat" name="total_berat" required step="0.01" min="0.01">
                        </div>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end">
                                        <li>
                                            <a class="custom-dropdown-item d-flex align-items-center  justify-content-start" data-bs-toggle="modal" data-bs-target="#editOrderModal{{ $order->id }}">
                                        ...
                                        <li>
                                            <form id="deleteForm" action="" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a type="button" class="custom-dropdown-item d-flex align-items-center  justify-content-start" data-bs-toggle="modal" data-bs-target="#deleteModal" data-order-id="{{ $order->id }}">
...
<!-- Modal -->
@foreach ($orders as $order)
<div class="modal fade" id="editOrderModal{{ $order->id }}" tabindex="-1" aria-labelledby="editOrderModalLabel{{ $order->id }}" aria-hidden="true" data-bs-backdrop="static" data-bs-keyboard="false">
                    ...
                    <div class="tab-content" id="editOrderTabContent{{ $order->id }}">
                                    ...
                                    <select id="toko_id{{ $order->id }}" name="toko_id" class="form-control" required
                                        ...
                                        @foreach($tokos as $toko)
                                            <option value="{{ $toko->id }}" data-marketplace="{{ $toko->marketplace }}"
                                                {{ $order->toko_id == $toko->id ? 'selected' : '' }}>{{ $toko->nama_toko }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="asal_penjualan{{ $order->id }}" class="form-label">Marketplace</label>
                                    <input type="text" class="form-control" id="asal_penjualan{{ $order->id }}" name="asal_penjualan" 
                                           value="{{ $order->asal_penjualan }}" readonly>
                                </div>
                            </div>
                        ...
                        <div class="tab-pane fade" id="shipping-info{{ $order->id }}" role="tabpanel" aria-labelledby="shipping-info-tab{{ $order->id }}">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <input type="text" class="form-control" id="provinsi{{ $order->id }}" name="provinsi" value="{{ $order->provinsi }}" required>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="tab-pane fade" id="payment-notes{{ $order->id }}" role="tabpanel" aria-labelledby="payment-notes-tab{{ $order->id }}">
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="total_berat{{ $order->id }}" name="total_berat" value="{{ $order->total_berat }}" required step="0.01" min="0.01">
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        $('#deleteModal').on('show.bs.modal', function(event) {
            ...
            var actionUrl = '{{ url("orders") }}/' + orderId; // Ganti dengan URL yang sesuai untuk penghapusan order
    ...
    // Panggil fungsi ini saat halaman dimuat agar sesuai dengan toko yang dipilih
    document.addEventListener("DOMContentLoaded", function() {
        @foreach($orders as $order)
            updateMarketplace({{ $order->id }});
        @endforeach
    });
