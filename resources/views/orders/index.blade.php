        ...
        <div class="card-body-order">
            ...
            <ul class="nav nav-tabs nav-fill mb-4 d-flex flex-nowrap overflow-auto" id="orderTab" role="tablist">
                <li class="nav-item flex-grow-1" role="presentation">
                    <button class="nav-link active text-center" id="order-info-tab" data-bs-toggle="tab" data-bs-target="#order-info" 
                        type="button" role="tab" aria-controls="order-info" aria-selected="true">Informasi Order</button>
                </li>
                <li class="nav-item flex-grow-1" role="presentation">
                    <button class="nav-link text-center" id="shipping-info-tab" data-bs-toggle="tab" data-bs-target="#shipping-info"
                        type="button" role="tab" aria-controls="shipping-info" aria-selected="false">Informasi Customer</button>
                </li>
                <li class="nav-item flex-grow-1" role="presentation">
                    <button class="nav-link text-center" id="payment-notes-tab" data-bs-toggle="tab" data-bs-target="#payment-notes"
                        type="button" role="tab" aria-controls="payment-notes" aria-selected="false">Informasi Pembayaran</button>
                </li>
            </ul>
                            ...
                            <td class="text-center">
                                ...
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu">
                                        <li>
                                            <a class="custom-dropdown-item" data-bs-toggle="modal" data-bs-target="#editOrderModal{{ $order->id }}">
                                        ...
                                        <li>
                                            <form id="deleteForm" action="" method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a type="button" class="custom-dropdown-item" data-bs-toggle="modal" data-bs-target="#deleteModal" data-order-id="{{ $order->id }}">
...
<!-- Modal -->
@foreach ($orders as $order)
<div class="modal fade" id="editOrderModal{{ $order->id }}" tabindex="-1" aria-labelledby="editOrderModalLabel{{ $order->id }}" 
    ...
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <ul class="nav nav-tabs nav-fill mb-4 d-flex flex-nowrap overflow-auto" id="EditOrderTab{{ $order->id }}" role="tablist">
                    <li class="nav-item flex-grow-1" role="presentation">
                        <button class="nav-link active text-center" id="order-info-tab{{ $order->id }}" data-bs-toggle="tab" data-bs-target="#order-info{{ $order->id }}" 
                            type="button" role="tab" aria-controls="order-info" aria-selected="true">Informasi Order</button>
                    </li>
                    <li class="nav-item flex-grow-1" role="presentation">
                        <button class="nav-link text-center" id="shipping-info-tab{{ $order->id }}" data-bs-toggle="tab" data-bs-target="#shipping-info{{ $order->id }}"
                            type="button" role="tab" aria-controls="shipping-info" aria-selected="false">Informasi Customer</button>
                    </li>
                    <li class="nav-item flex-grow-1" role="presentation">
                        <button class="nav-link text-center" id="payment-notes-tab{{ $order->id }}" data-bs-toggle="tab" data-bs-target="#payment-notes{{ $order->id }}"
                            type="button" role="tab" aria-controls="payment-notes" aria-selected="false">Informasi Pembayaran</button>
                    </li>
                </ul>
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select id="toko_id{{ $order->id }}" name="toko_id" class="form-control" required onchange="updateMarketplace({{ $order->id }})">
                                        @foreach ($tokos as $toko)
                                        <option value="{{ $toko->id }}" data-marketplace="{{ $toko->marketplace }}" {{ $order->toko_id == $toko->id ? 'selected' : '' }}>
                                            {{ $toko->nama_toko }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="tab-pane fade" id="shipping-info{{ $order->id }}" role="tabpanel" aria-labelledby="shipping-info-tab{{ $order->id }}">
                            ...
                            <div class="row">
                                <div class="col-md-6 mb-3">
                                    ...
                                    <select class="form-control" id="provinsi{{ $order->id }}" name="provinsi" value="{{ $order->provinsi }}" required>
                                </div>
                                <div class="col-md-6 mb-3">
                                    <label for="kota{{ $order->id }}" class="form-label">Kota</label>
                                    <input type="text" class="form-control" id="kota{{ $order->id }}" name="kota" value="{{ $order->kota }}" required>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6 mb-3">
                        ...
                        <div class="tab-pane fade" id="payment-notes{{ $order->id }}" role="tabpanel" aria-labelledby="payment-notes-tab{{ $order->id }}">
                            <div class="mt-3">
                                ...
                                <div id="buku-container-{{ $order->id }}">
                                    ...
                                    <div class="row align-items-center mb-2 buku-row" id="buku-row-{{ $order->id }}-{{ $index }}">
                                        ...
                                        <div class="col-2 col-md-3">
                                            <input type="text" name="bukus[{{ $index }}][jumlah]" class="form-control jumlah-input" 
                                                value="{{ $buku->pivot->jumlah }}" placeholder="Jumlah" required>
                                        </div>
                                        <div class="col-3 col-md-2 d-flex justify-content-between">
                                            <i class="bi bi-plus-circle text-primary fs-4 cursor-pointer add-buku" title="Tambah Buku" data-order-id="{{ $order->id }}"></i>
                                            @if ($index > 0)
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" class="form-control" id="total_berat{{ $order->id }}" name="total_berat" value="{{ $order->total_berat }}" required step="0.01" min="0.01" readonly>
                            </div>

                            <div class="mb-3">
                                <label for="grand_total{{ $order->id }}" class="form-label">Grand Total</label>
                                <input type="number" step="0.01" class="form-control" id="grand_total{{ $order->id }}" name="grand_total" value="{{ $order->grand_total }}" required readonly>
                            </div>
